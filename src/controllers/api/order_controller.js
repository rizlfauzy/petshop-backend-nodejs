      ...
      for (const barang of list_barang) {
        if (selisih_bulan > 0) {
          ...
          for (let i = 0; i <= selisih_bulan; i++) {
            ...
            if (stock) {
              const data_update_stock = i == 0 ? { qty_masuk: stock.qty_masuk + barang.qty } : { qty_awal };
              await inventory_barang.update(
                ...
                { where: { barcode: barang.barcode, periode: periode_stock }, transaction }
              );
              const stock_now = await inventory_barang.findOne({ where: { barcode: barang.barcode, periode: periode_stock } }, { transaction });
              qty_awal = stock_now.qty_awal + stock_now.qty_masuk - stock_now.qty_keluar;
            } else {
              const data_insert_stock = i == 0 ? { qty_masuk: barang.qty, qty_awal: 0 } : { qty_awal, qty_masuk: 0 };
          ...
          if (stock) {
            await inventory_barang.update(
              { qty_masuk: stock.qty_masuk + barang.qty, tglupdate: moment().format("YYYY-MM-DD HH:mm:ss"), pemakai: req.user.myusername.toUpperCase() },
